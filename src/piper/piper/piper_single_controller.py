...
class PiperRosNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # Joint states initialization with conditional gripper
        ...
        # Joint ctrl initialization with conditional gripper
        ...
        if self.gripper_exist:
            ...
            self.joint_ctrl.name = ['joint1', 'joint2', 'joint3', 'joint4', 'joint5', 'joint6']
            self.joint_ctrl.position = [0.0] * 6
            self.joint_ctrl.velocity = [0.0] * 6
            self.joint_ctrl.effort = [0.0] * 6
    ...
    def PublishArmJointAndGripper(self):
        ...
        if self.gripper_exist:
            ...
            # No gripper data
            self.joint_states.position = [joint_0, joint_1, joint_2, joint_3, joint_4, joint_5]
            self.joint_states.velocity = [vel_0, vel_1, vel_2, vel_3, vel_4, vel_5]
            self.joint_states.effort = [effort_0, effort_1, effort_2, effort_3, effort_4, effort_5]
    ...
    def PublishArmCtrlAndGripper(self):
        ...
        # Get joint control positions (first 6 joints)
        ...
        if self.gripper_exist:
            ...
            # No gripper control data
            self.joint_ctrl.position = [joint_0, joint_1, joint_2, joint_3, joint_4, joint_5]
    ...
    def joint_callback(self, joint_data):
        ...
        # Create a dictionary to store joint name to position mapping
        joint_positions = {}
        joint_6 = 0

        # Map positions based on joint names (only process first 6 joints for arm)
        ...
        # Get gripper position if available and gripper exists
        if self.gripper_exist and len(joint_data.position) >= 7:
            # Look for gripper/joint7 in the joint names
            gripper_idx = -1
            for idx, joint_name in enumerate(joint_data.name):
                if joint_name in ['gripper', 'joint7']:
                    gripper_idx = idx
                    break
            
            if gripper_idx >= 0:
                joint_6 = round(joint_data.position[gripper_idx] * 1000 * 1000)
        ...
        if self.GetEnableFlag():
            ...
            if not all_zeros:
                ...
                if self.gripper_exist and lens >= 7:
            ...
            # Use joint names to dynamically control joints
            ...
            # Gripper control (only if gripper exists)
            if self.gripper_exist:
